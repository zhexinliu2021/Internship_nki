...
# Usage: python3 -t int -o out_path
# In[146]:
...
path = '/scratch/CCLE/mutation_files/'
file_list = ['damaging','hotspot','nonconserving','otherconserving']
drug_path = '/scratch/CCLE/primary-screen-replicate-collapsed-logfold-change.csv'
#out_path = '/Users/jerryliu/jerry_jupyter/internship/output/'
Con_file='/scratch/CCLE/mutation_files/Census_allSat.csv'
...
def out_loop (x, y_total, cl_id, out_cv_fold= 5):
    ...
    for train, test in kf.split(x):
        ...
        alpha_list =  np.array([math.exp(i) for i in np.arange(-150,15,20)] ) # 250 values
...
def drug_model(m_file, target, target_index, out_path):
    """
    ...
    tools.write_file(Y_pre_array, out_path, 'comp{}.csv'.format(target_index+1) )
...
# In[48]:
    ...
    gene_lists =  [ m_file.columns.map(lambda x:x.split(' ')[0])
        for m_file in [m_f1, m_f2, m_f3] ]
    final_genes = tools.get_cons(tools.get_cons(*gene_lists, method='union'),
                   Con_genes, method = 'inter')
...
# In[50]:
    ...
    #take genes intersecting the COSMIC data


    m_file = m_file.loc[:,final_genes]
    print(m_file.shape)
    ...
    #print(f'after filtering, the number of drugs is {len(drug_id_list)}')


# In[ ]:
    comp_index = dg_index
    target = drug_df.iloc[:, comp_index]
    pre = drug_model(m_file, target, comp_index, out_path=out_path)
    ...
    # drug_mapping_list = (list(zip(drug_id_list, 
    #                           [np.where(drug_data.columns == drug_id) for drug_id in drug_id_list])))
...
if __name__ == '__main__':
    ...
    dg_index = (args.target_index -1 ) #convert to 0-based
